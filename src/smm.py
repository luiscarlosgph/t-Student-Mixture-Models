...
class SMM(sklearn.base.BaseEstimator):
    ...
    def _expectation_step(self, X):
        ...
        if X.shape[1] != self.means_.shape[1]:
            raise ValueError(
                '[SMM._expectation_step] Error, the shape of X is not compatible with self.')

        # Initialisation of reponsibilities and weight of each point for the Gamma distribution
        n_samples, n_dim = X.shape
        responsibilities = np.ndarray(
            shape=(X.shape[0], self._n_components), dtype=np.float64)
        ...
        responsibilities = pr / \
            (likelihoods.reshape(likelihoods.shape[0], 1) + 10 * SMM._EPS)

        # Update the Gamma weight for each observation
        mahalanobis_distance_mix_func = SMM._mahalanobis_funcs[self._covariance_type]
        gammaweights_ = (self.degrees_ + n_dim) / (self.degrees_ +
                                                   mahalanobis_distance_mix_func(X, self.means_, self.covars_, self._min_covar))
    ...
    def _maximisation_step(self, X, responsibilities, gammaweights_):
        ...
        if 'm' in self._params:
            self.means_ = np.dot(
                zu.T, X) / (zu_sum.reshape(zu_sum.shape[0], 1) + 10 * SMM._EPS)

        # Update covariances
        if 'c' in self._params:
            covar_mstep_func = SMM._covar_mstep_funcs[self._covariance_type]
            self.covars_ = covar_mstep_func(
                X, zu, z_sum, self.means_, self._min_covar)

        # Update degrees of freedom
        if 'd' in self._params:
            try:
                self.degrees_ = SMM._solve_dof_equation(
                    self.degrees_, responsibilities, z_sum, gammaweights_, n_dim, self._tol, self._n_iter)
    ...
    def fit(self, X, y=None):
        ...
        if (len(X.shape) == 1):
            raise ValueError(
                'The input matrix must have a shape of (n_samples, n_features).')
        ...
        if X.shape[0] < self._n_components:
            raise ValueError('SMM estimation with %s components, but got only %s samples' % (
                self._n_components, X.shape[0]))
        ...
        for _ in range(self._n_init):
            ...
            if 'm' in self._init_params or not hasattr(self, 'means_'):
                self.means_ = sklearn.cluster.KMeans(
                    n_clusters=self._n_components, init='k-means++', random_state=self._random_state).fit(X).cluster_centers_

            if 'w' in self._init_params or not hasattr(self, 'weights_'):
                self.weights_ = np.tile(
                    1.0 / self._n_components, self._n_components)

            if 'c' in self._init_params or not hasattr(self, 'covars_'):
                cv = np.cov(X.T) + self._min_covar * np.eye(X.shape[1])
                if not cv.shape:
                    cv.shape = (1, 1)
                self.covars_ = SMM.distribute_covar_matrix_to_match_covariance_type(
                    cv, self._covariance_type, self._n_components)
            ...
            for i in xrange(self._n_iter):
                ...
                # Expectation step
                likelihoods, responsibilities, gammaweights_ = self._expectation_step(
                    X)
    ...
    def score_samples(self, X):
        ...
        if X.shape[1] != self.means_.shape[1]:
            raise ValueError(
                '[score_samples] ValueError, the shape of X  is not compatible with self.')
    ...
    def _multivariate_t_student_density_diag(X, means, covars, dfs, min_covar):
        ...
        braces = 1.0 + maha / dfs
        denom = np.power(np.pi * dfs, n_dim / 2.0) * scipy.special.gamma(dfs / 2.0) * \
            np.sqrt(det_covars) * np.power(braces, (dfs + n_dim) / 2.0)
    ...
    def _multivariate_t_student_density_full(X, means, covars, dfs, min_covar):
        ...
        for c, (mu, cv, df) in enumerate(zip(means, covars, dfs)):
            ...
            gamma_coef = np.exp(scipy.special.gammaln(
                (r + n_dim) / 2.0) - scipy.special.gammaln(r / 2.0))

            # Calculate the denominator of the multivariate t-Student
            denom = np.sqrt(cov_det) * np.power(np.pi * df, n_dim /
                                                2.0) * np.power((1 + maha / df), (df + n_dim) / 2)
        ...
        if covariance_type == 'full':
            ...
            for n, cv in enumerate(covars):
                if (not np.allclose(cv, cv.T) or np.any(linalg.eigvalsh(cv) <= 0)):
                    raise ValueError(
                        "component %d of 'full' covars must be symmetric, positive-definite" % n)
                else:
                    raise ValueError(
