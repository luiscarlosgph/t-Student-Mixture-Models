#!/usr/bin/env python
#
# @brief  Example that shows how to generate a random sample of data coming from 
#         a mixture of t-Student distributions and then use the 'smm.SMM' class 
#         to estimate the parameters of the original model using 
...
def main():
    ...
    vector_covars = np.random.randint(max_covar, 
            size=(n_components, int((n_dim + 1) * n_dim / 2)))
    ...
    obs = []
    for c in range(n_components):
        c_obs = smm.SMM.multivariate_t_rvs(means[c], covars[c], 
                                           df=dfs[c], n=n_samples)
