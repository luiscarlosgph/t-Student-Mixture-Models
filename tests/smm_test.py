...
class TestSMM(unittest.TestCase):
	...
	def test_univariate_standard_t_student_density_full(self):
		...
		X = np.array([[2]])
		dfs = np.array([7])
		proba = smm.SMM._multivariate_t_student_density_full(X, means, covars, dfs, min_covar)[0][0]
	...
	def test_multivariate_t_student_density_diag(self):
		...
		X = np.array([[21.9, 1.2, 120.1]])
		proba = smm.SMM._multivariate_t_student_density_diag(X, means, covars, dfs, min_covar)[0][0]
	...
	def test_multivariate_t_student_density_spherical(self):
		...
		X = np.array([[21.9, 1.2, 120.1]])
		proba = smm.SMM._multivariate_t_student_density_spherical(X, means, covars, dfs, min_covar)[0][0]
	...
	def test_one_component_em(self):
		...
		for i in range(min_df, max_df):
			...
			# np.random.seed()
			obs = smm.SMM.multivariate_t_rvs(means[0], covars[0], df=dfs[0], n=n_samples)
			...
			cov_error = np.fabs(np.linalg.norm(covars[0] - mix_t.covariances[0]))
	...
	def test_several_component_em(self):
		...
		for k in range(min_components, max_components):
			...
			vector_covars = np.random.randint(100, size = (k, (n_dim + 1) * n_dim / 2))
			...
			for c in range(k):
				...
				cov_error = np.linalg.norm(covars[c] - mix_t.covariances[indices[c]], 'fro')
				self.assertTrue(cov_error < covars_eps)
